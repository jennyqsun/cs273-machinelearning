...
## METHODS ################################################################
    ...
    def train(self, X, Y, initStep=0.1, stopTol=1e-4, stopEpochs=5000, plot=None):
        ...
        #
        # plt.draw()
        plt.figure(3); plt.plot(Jnll,'b-',J01,'r-'); plt.title('Surrogate Loss and Error Rate');plt.draw();    # plot losses
        plt.figure(4);plt.title('Converged');self.plotBoundary(X,Y); plt.draw();



    def trainl2(self, X, Y, initStep=0.1, stopTol=1e-4, stopEpochs=5000, plot=None, alpha=2):
        """ Train the logistic regression using stochastic gradient descent """
        M,N = X.shape;                     # initialize the model if necessary:
        self.classes = np.unique(Y);       # Y may have two classes, any values
        XX = np.hstack((np.ones((M,1)),X)) # XX is X, but with an extra column of ones
        ...
        while not done:
            ...
            for i in np.random.permutation(M):
                ri    = self.theta@XX[i,:];     # TODO: compute linear response r(x)
                gradi = (-YY[i] + sigma(ri)) * XX[i,:]
                gradi += 2*alpha*self.theta# TODO: compute gradient of NLL loss
                self.theta -= stepsize * gradi;  # take a gradient step
